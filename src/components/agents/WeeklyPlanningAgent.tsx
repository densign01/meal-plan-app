...
export default function WeeklyPlanningAgent({
    ...
    const continueMutation = useMutation({
        mutationFn: ({ sessionId, message, history }: { sessionId: string; message: string; history: ChatMessage[] }) =>
            MealPlanAPI.continueWeeklyPlanning(sessionId, message, history),
    ...
    const handleSendMessage = (message: string) => {
        ...
        // Add user message to chat and get updated history
        const updatedMessages: ChatMessage[] = [...messages, { role: 'user' as const, content: message }]
        setMessages(updatedMessages)
        console.log('📊 Messages after adding user message:', updatedMessages.length)

        // Send to backend with history
        continueMutation.mutate({ sessionId, message, history: updatedMessages })
    }
